...
typedef StrumNoteType = #if (PSYCH || LEATHER) StrumNote #elseif KADE FlxSprite #elseif FOREVER_LEGACY UIStaticArrow #elseif ANDROMEDA Receptor #else FlxSprite #end;
...
class PlayfieldRenderer extends FlxSprite // extending flxsprite just so i can edit draw
{
	...
	private function drawStuff(notePositions:Array<NotePositionData>)
	{
		for (noteData in notePositions)
		{
			...
			else if (!notes.members[noteData.index].isSustainNote) // draw regular note
			{
				...
				var thisNotePos = ModchartUtil.calculatePerspective(new Vector3D(noteData.x + (daNote.width / 2), noteData.y + (daNote.height / 2),
					noteData.z * 0.001),
					ModchartUtil.defaultFOV * (Math.PI / 180),
					-(daNote.width / 2),
					-(daNote.height / 2));
			...
			else // draw sustain
			{
				...
				var thisNotePos = ModchartUtil.calculatePerspective(new Vector3D(noteData.x
					+ (daNote.width / 2)
					+ ModchartUtil.getNoteOffsetX(daNote),
					noteData.y
	...
	public function tweenModifier(modifier:String, val:Float, time:Float, ease:String)
	{
		if (modifiers.exists(modifier))
		{
			...
			#if PSYCH
			instance.modchartTweens.set(modifier, FlxTween.tween(modifiers.get(modifier), {currentValue: val}, time, {
				ease: easefunc,
			...
			#else
			FlxTween.tween(modifiers.get(modifier), {currentValue: val}, time, {
				ease: easefunc,
				onComplete: function(twn:FlxTween)
				{
				}
	...
	public function tweenModifierSubValue(modifier:String, subValue:String, val:Float, time:Float, ease:String)
	{
		if (modifiers.exists(modifier))
		{
			if (modifiers.get(modifier).subValues.exists(subValue))
			{
				...
				#if PSYCH
				instance.modchartTweens.set(tag, FlxTween.num(startValue, val, time, {
					ease: easefunc,
					onComplete: function(twn:FlxTween)
					{
				...
				#else
				FlxTween.num(startValue, val, time, {
					ease: easefunc,
					onComplete: function(twn:FlxTween)
					{
						if (modifiers.exists(modifier))
